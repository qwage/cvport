...
#INITILATION
FOV = 90 *np.pi/180 #90 degree horizontal field of view in radians
w_frame  = 200 #pixel width of frame
dt = 0.1 #seconds between frames for 10 frames/s 
veh_speed2 = 1 #m/s from  wheels
veh_speed1 = 1 
veh_angle2 = np.pi/2 #rad where pi/2 is north (+y dir.)\
veh_angle1 = np.pi/2
...
x_2 = r2*np.cos(veh_angle2-theta2)
x_1 = r1*np.cos(veh_angle1-theta1)
y_2 = r2*np.sin(veh_angle2-theta2)
y_1 = r1*np.sin(veh_angle1-theta1)
v_rel =  np.array([(x_2-x_1)/dt,(y_2-y_1)/dt])

v_veh_x = .5*(veh_speed2*np.cos(veh_angle2) + veh_speed1*np.cos(veh_angle1))
v_veh_y = .5*(veh_speed2*np.sin(veh_angle2) + veh_speed1*np.sin(veh_angle1))
